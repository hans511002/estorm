...
public final class ByteBufferUtils {
	...
	/**
	 * Similar to {@link WritableUtils#writeVLong(java.io.DataOutput, long)}, but writes to a {@link ByteBuffer}.
	 */
	...
	/**
	 * Put in buffer integer using 7 bit encoding. For each written byte: 7 bits are used to store value 1 bit is used to indicate whether
	 * there is next bit.
	 * 
	 * @param value
	 ...
	 *            the buffer to read bytes from
	 * @param offset
	 *            the offset in the buffer (from the buffer's array offset) to start copying bytes from
	 * @param length
	 *            the number of bytes to copy
	 */
	...
	/**
	 * Copy the given number of bytes from the given stream and put it at the current position of the given buffer, updating the position in
	 * the buffer.
	 * 
	 * @param out
	...
	public static boolean arePartsEqual(ByteBuffer buffer, int offsetLeft, int lengthLeft, int offsetRight, int lengthRight) {
		...
		if (buffer.hasArray()) {
			return 0 == Bytes.compareTo(buffer.array(), buffer.arrayOffset() + offsetLeft, lengthLeft, buffer.array(), buffer.arrayOffset()
	...
	/**
	 * Copy the bytes from position to limit into a new byte[] of the exact length and sets the position and limit back to their original
