...
/**
 * Sleeper for current thread. Sleeps for passed period. Also checks passed boolean and if interrupted, will return if the flag is set
 * (rather than go back to sleep until its sleep time is up).
 */
public class Sleeper {
	 ...
	 * 
	 * @param startTime
	 *            Time some task started previous to now. Time to sleep will be docked current time minus passed <code>startTime<code>.
	 */
	public void sleep(final long startTime) {
		...
		if (waitTime > this.period) {
			LOG
					.warn("Calculated wait time > " + this.period + "; setting to this.period: " + System.currentTimeMillis() + ", "
							+ startTime);
