...
/**
 * Create a Jetty embedded server to answer http requests. The primary goal is to serve up status information for the server. There are
 * three contexts: "/stacks/" -> points to stack trace "/static/" -> points to common static files (src/hbase-webapps/static) "/" -> the jsp
...
public class InfoServer extends HttpServer {
	...
	/**
	 * Create a status server on the given port. The jsp scripts are taken from src/hbase-webapps/<code>name<code>.
	 * 
	 * @param name
	 ...
	 * @param findPort
	 *            whether the server should start at the given port and increment by 1 until it finds a free port.
	 * @throws IOException
	 *             e
	 */
	public InfoServer(String name, String bindAddress, int port, boolean findPort, final Configuration c) throws IOException {
	...
	/**
	 * Fixup where the logs app points, make it point at hbase logs rather than hadoop logs.
	 */
	...
	protected String getWebAppsPath(String appName) throws FileNotFoundException {
		...
		String resourceName = "estorm-webapps/" + appName;
