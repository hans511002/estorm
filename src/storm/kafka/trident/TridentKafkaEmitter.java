...
/**
 * Date: 21/05/2013 Time: 08:38
 */
public class TridentKafkaEmitter {

	public static final Log LOG = LogFactory.getLog(TridentKafkaEmitter.class);
	...
	private Map failFastEmitNewPartitionBatch(TransactionAttempt attempt, TridentCollector collector,
	...
	private Map emitNewPartitionBatch(TransactionAttempt attempt, TridentCollector collector, Partition partition,
			Map lastMeta) {
		try {
	...
	private Map doEmitNewPartitionBatch(SimpleConsumer consumer, Partition partition, TridentCollector collector,
			Map lastMeta) {
		long offset;
		if (lastMeta != null) {
	...
	public IOpaquePartitionedTridentSpout.Emitter<GlobalPartitionInformation, Partition, Map> asOpaqueEmitter() {
		...
		return new IOpaquePartitionedTridentSpout.Emitter<GlobalPartitionInformation, Partition, Map>() {
			 ...
			 */
			@Override
			public Map emitPartitionBatch(TransactionAttempt transactionAttempt, TridentCollector tridentCollector,
	...
	public IPartitionedTridentSpout.Emitter asTransactionalEmitter() {
		return new IPartitionedTridentSpout.Emitter<GlobalPartitionInformation, Partition, Map>() {
			...
			/**
			 * Emit a batch of tuples for a partition/transaction that's never
			 * been emitted before. Return the metadata that can be used to
			 * reconstruct this partition/batch in the future.
			 */
			@Override
			public Map emitPartitionBatchNew(TransactionAttempt transactionAttempt, TridentCollector tridentCollector,
					Partition partition, Map map) {
			 ...
			 * emitted before, using the metadata created when it was first
			 * emitted.
			 */
			@Override
			 ...
			 * of partitions. Should be used to manage things like connections
			 * to brokers.
			 */
			@Override
			public void refreshPartitions(List<Partition> partitions) {
